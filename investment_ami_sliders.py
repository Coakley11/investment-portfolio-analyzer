...
def _resolve_deploy_marker() -> str:
    ...
    try:
        ...
        pass
    try:
        from suite_analytical_question import AMI_SIDEBAR_DEPLOY_VERSION

        return str(AMI_SIDEBAR_DEPLOY_VERSION)
    except Exception:
        return "unknown"


def _render_slider_runtime_debug_probe(
    st: Any,
    insight_data: dict[str, Any],
    *,
    problem_type: str,
    ...
    """TEMP: ``?slider_debug=1`` — remove after runtime verification."""
    if not _query_flag(st, "slider_debug"):
        return

    import investment_ami_macro as macro_mod
    import investment_ami_sliders as sliders_mod

    st.markdown("---")
    st.markdown("**TEMP slider runtime debug** (`?slider_debug=1`)")
    st.code(
        "\n".join(
            [
                f"investment_ami_sliders.__file__ = {getattr(sliders_mod, '__file__', '?')}",
                f"investment_ami_macro.__file__ = {getattr(macro_mod, '__file__', '?')}",
                f"deploy_marker = {_resolve_deploy_marker()}",
            ]
        ),
        language="text",
    )
    ...
    for spec in specs:
        st.markdown(
            f"- **SliderSpec:** key=`{spec.key}` label=`{spec.label}` "
            f"min=`{spec.minimum}` max=`{spec.maximum}` default=`{spec.default}`"
        )

    has_shock = hasattr(macro_mod, "parse_rate_shock_pp")
    has_rise = hasattr(macro_mod, "parse_rate_rise_pct")
    st.markdown(
        f"- **Parser symbols:** parse_rate_shock_pp={has_shock} · parse_rate_rise_pct={has_rise}"
    )
    if has_shock:
        parser_name = "parse_rate_shock_pp"
        parser_fn = macro_mod.parse_rate_shock_pp
    elif has_rise:
        parser_name = "parse_rate_rise_pct"
        parser_fn = macro_mod.parse_rate_rise_pct
    else:
        parser_name = "(none)"
        parser_fn = None

    st.markdown(f"- **Active parser:** `{parser_name}` (module: `{macro_mod.__name__}`)")

    effective: dict[str, Any] = dict(session_params)
    insight_scenario = insight_data.get("scenario_params")
    if isinstance(insight_scenario, dict):
        effective = {**effective, **insight_scenario}
    kn = insight_data.get("key_numbers")
    if isinstance(kn, dict) and isinstance(kn.get("scenario_params"), dict):
        effective = {**effective, **kn["scenario_params"]}

    question = str(insight_data.get("question") or "").strip()
    parsed: Any = None
    if parser_fn is not None and problem_type == "macro_rates" and question:
        try:
            parsed = parser_fn(question, scenario_params=effective)
        except TypeError:
            parsed = parser_fn(question)
        except Exception as exc:
            parsed = f"error: {type(exc).__name__}: {exc}"

    st.markdown("- **Effective scenario_params (solver input):**")
    st.json(effective)
    if problem_type == "macro_rates":
        st.markdown(f"- **Parser output for this question:** `{parsed!r}`")
    st.markdown("---")
...
def render_ami_assumption_controls(st: Any, insight_data: dict[str, Any]) -> bool:
    ...
    _render_slider_runtime_debug_probe(
        st,
        insight_data,
